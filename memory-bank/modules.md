...
`src/AgentClient/Workflow/MagenticWorkflow.cs` — адаптер встроенной Magentic-оркестрации.
Он отвечает за создание workflow, события plan/progress/replan и передачу финального
workspace evidence typed Reviewer. Он не изменяет MCP Server и не содержит предметной
логики.
